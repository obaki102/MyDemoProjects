...
{
    public class CreateAccountRequestFluentValidator : AbstractValidator<CreateAccountRequest>
    {
        public CreateAccountRequestFluentValidator()
        {
            RuleFor(x => x.Email)
        ...
        {
            var result = await ValidateAsync(ValidationContext<CreateAccountRequest>.CreateWithOptions((CreateAccountRequest)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
